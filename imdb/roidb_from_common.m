...
try
  ...
  fprintf('done\n');
  if( strcmp(roidb.name,'unlabel') )
      for i = 1:length(imdb.image_ids)
        tic_toc_print('roidb (%s): %d/%d\n', roidb.name, i, length(imdb.image_ids));
        
        roidb.rois(i) = my_attach_proposals(regions.boxes{i}, experiment_dir);
      end
  else 
      for i = 1:length(imdb.image_ids)
        tic_toc_print('roidb (%s): %d/%d\n', roidb.name, i, length(imdb.image_ids));
...
%%%%%%%%%%%%%%%%%
function rec = my_attach_proposals(boxes, experiment_dir)
    boxes = boxes(:, [2 1 4 3 5]);
...
%        class: [2108x1 uint8]
    ...
    rec.overlap = ones(num_boxes, 1, 'single');
